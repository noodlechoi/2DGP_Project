...
def handle_events():
    ...
    for event in events:
        if event.type == SDL_QUIT:
            ...
            # exit를 누르면 종료
            if is_conflict([location[3][0], location[3][1] - 50], [200, 100], [event.x, HEIGHT - event.y]):
                game_framework.quit()
            elif is_conflict([location[3][0], location[3][1] + 50], [300, 100], [event.x, HEIGHT - event.y]):
